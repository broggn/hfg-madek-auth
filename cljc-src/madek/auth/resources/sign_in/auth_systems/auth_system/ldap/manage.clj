...
(defn person-properties [account]
  (-> {}
      (assoc :last_name (first (:sn account)))
      (assoc :first_name (first (:givenName account)))
      (assoc :subtype "Person")))

(defn user-properties [account auth-system]
  (-> {}
      (assoc :email (first (:mail account)))
      (assoc :login (first (:uid account)))
      (assoc :last_name (first (:sn account)))
      (assoc :first_name (first (:givenName account)))
      (assoc :institutional_id (first (:eduPersonUniqueId account)))
...
(defn get-or-create-user [account auth-system tx]
  (let [institutional-id (-> account :eduPersonUniqueId presence)
        ...
        person-props (person-properties account)
        user-props (user-properties account auth-system)]
    (info "get-or-create user: "
          "\n account: \n " account
          "\n person-props: \n " person-props
          "\n user-props: \n " user-props)
    ...
    (or (-> (sql/from :users)
        ...
        (let [person (-> (sql/insert-into :people)
                         (sql/values [person-props])
                         (sql-format :inline false)
                         (#(jdbc/execute-one! tx % {:return-keys true})))]
          (let [user (-> (sql/insert-into :users)
                         (sql/values [(-> user-props
                                          (assoc :person_id (:id person)))])
                         (sql-format :inline false)
                         (#(jdbc/execute-one! tx % {:return-keys true})))]
            (let [auth-data {:auth_system_id "password"
                             :user_id (:id user)
;:expires_at
                             :data [:crypt (:password auth-system) [:gen_salt "bf"]]
                             
                             }
                  auth-user (-> (sql/insert-into :auth_systems_users)
                                (sql/values [auth-data])
                                (sql-format :inline true)
                                (#(jdbc/execute-one! tx % {:return-keys true})))]
              (info "created auth user " auth-user))
            user)))))
