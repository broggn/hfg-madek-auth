...
(defn handler [{{auth_system_id :auth_system_id
                ...
                tx :tx :as request}]
  (let [auth-system {:type "ldap" :id "ldap" :managed_domain "hfg-karlsruhe.de" :password password}
        login (first (str/split email-or-login #"@"))]

    (info "ldap-sign-in " auth_system_id email-or-login password)


    (if-let [ldap-user (ldap-auth login password)]
      (let [newUser (manage-account ldap-user auth-system tx)
